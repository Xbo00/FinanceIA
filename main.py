...
+    help="Esto evita recomendaciones de venta cuando aun no tienes una compra activa."
+)
...
@@ -62,52 +67,62 @@ def create_signal(pct_change):
...
+prediccion_hoy = model.predict(ultima_fila)
+prediccion_valor = int(prediccion_hoy[0])
...
-else:
-    estado = "ESPERAR / HOLD"
+if prediccion_valor == 1:
+    if estado_posicion == "Sin posicion abierta":
+        estado = "COMPRAR"
+    else:
+        estado = "MANTENER (ya estas dentro)"
+elif prediccion_valor == -1:
+    if estado_posicion == "Con posicion abierta":
+        estado = "VENDER"
+    else:
+        estado = "ESPERAR (sin posicion para vender)"
+else:
+    if estado_posicion == "Con posicion abierta":
+        estado = "MANTENER / HOLD"
+    else:
+        estado = "ESPERAR / HOLD"
